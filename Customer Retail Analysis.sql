...
SELECT Product_Name → selects each product

AVG(Review_Rating) → calculates average customer rating per product

FROM clean_retal → source table

GROUP BY Product_Name → aggregates ratings by product

ORDER BY avg_rating DESC → highest-rated products first
